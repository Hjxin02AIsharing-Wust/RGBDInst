...
@PIPELINES.register_module()
class LoadImageandDepthFromFile:
    """Load an image from file.
    ...
    def __call__(self, results):
        ...
        filename_jpg = osp.basename(filename.split('/')[-1])
        if osp.basename(filename.split('/')[-2]) == 'images':
            # print('virtual!')
            path_last1 = osp.dirname(filename)
            path_last2 = osp.dirname(path_last1)
            path_base = osp.dirname(path_last2)
        else:
            # print('real!')
            path_last1 = osp.dirname(filename)
            path_base = osp.dirname(path_last1)
        path_depth = osp.join(path_base, 'depth', filename_jpg[:-3] + 'png')

        img_bytes = self.file_client.get(filename)    #一堆十六进制
        img = mmcv.imfrombytes(
            img_bytes, flag=self.color_type, channel_order=self.channel_order)
        # img = ndarray<(480,640,3), uint8>

        depth_bytes = self.file_client.get(path_depth)    #一堆十六进制
        depth = mmcv.imfrombytes(
            depth_bytes, flag='unchanged')
        k = 0.00025
        depth = depth*k
        ...
        # img = ndarray<(480,640,3), float32>

        # img = np.stack([img, depth], axis=-1)
        img = np.concatenate([img, depth], axis = -1)   # img = ndarray<(480,640,4)>
        # print("!!!!!!!!!!!!!!", img)
